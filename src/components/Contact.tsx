...
import { Mail, Phone, Check, ArrowRight, Sparkles, AlertCircle, ShoppingBag, Globe, Smartphone, Code, Cpu, Layers, Video, Palette, Cloud, TrendingUp, Users } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';

const ecommerceServices = [
  { id: 'amazon',       label: 'Amazon FBA & FBM',          icon: ShoppingBag },
  { id: 'walmart',      label: 'Walmart Seller Growth',      icon: TrendingUp  },
  { id: 'ebay',         label: 'eBay Channel Strategy',      icon: Globe       },
  { id: 'shopify',      label: 'Premium Shopify Storefronts',icon: Sparkles    },
  { id: 'fb-marketplace', label: 'Facebook Shops',           icon: Users       },
];

const techServices = [
  { id: 'mobile-app',    label: 'Mobile App Development',    icon: Smartphone },
  { id: 'web-dev',       label: 'Website Development',       icon: Code       },
  { id: 'software-dev',  label: 'Custom Software / SaaS',    icon: Cpu        },
  { id: 'wordpress',     label: 'WordPress Development',     icon: Globe      },
  { id: 'ui-ux',         label: 'UI/UX Design',              icon: Layers     },
  { id: 'video-editing', label: 'Video Editing',             icon: Video      },
  { id: 'graphic-design',label: 'Graphic & Brand Design',    icon: Palette    },
  { id: 'aws',           label: 'AWS Cloud Services',        icon: Cloud      },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mt-8">
          ...
          <div className="lg:col-span-12 xl:col-span-7 backdrop-blur-xl rounded-3xl p-6 sm:p-8 space-y-6 text-left shadow-lg border" style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderColor: '#bae6fd' }}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    key={cat.id}
                    onClick={() => { setCategory(cat.id); setSelectedSubService(''); }}
            ...
            <AnimatePresence mode="wait">
              {(category === 'ecommerce' || category === 'tech') && (
                <motion.div
                  key={category}
                  ...
                  className="space-y-3"
                >
                  <span className="block font-sans text-xs font-bold uppercase tracking-wider" style={{ color: '#64748b' }}>
                    02. {category === 'ecommerce' ? 'E-Commerce Service' : 'Tech Service'}
                  </span>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {(category === 'ecommerce' ? ecommerceServices : techServices).map((svc) => {
                      const Icon = svc.icon;
                      const isActive = selectedSubService === svc.id;
                      return (
                        <button
                          key={svc.id}
                          onClick={() => setSelectedSubService(isActive ? '' : svc.id)}
                          className="flex items-center gap-2 p-3 rounded-xl border text-left font-sans text-xs font-semibold transition-all cursor-pointer"
                          style={isActive
                            ? { backgroundColor: '#e0f2fe', borderColor: '#0ea5e9', color: '#0c1a2e' }
                            : { backgroundColor: 'rgba(255,255,255,0.7)', borderColor: '#bae6fd', color: '#475569' }
                          }
                          onMouseEnter={e => { if (!isActive) e.currentTarget.style.backgroundColor = '#f0f9ff'; }}
                          onMouseLeave={e => { if (!isActive) e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.7)'; }}
                        >
                          <Icon className="w-3.5 h-3.5 shrink-0" style={{ color: isActive ? '#0ea5e9' : '#94a3b8' }} />
            ...
            {/* Tier Scope */}
            <div className="space-y-3">
              <span className="block font-sans text-xs font-bold uppercase tracking-wider" style={{ color: '#64748b' }}>03. Operational Tier Scope</span>
            ...
            {/* Specifications */}
            <div className="space-y-3">
              <span className="block font-sans text-xs font-bold uppercase tracking-wider" style={{ color: '#64748b' }}>04. Project Specifications & Integrations</span>
          ...
          <div id="contact-intake-form" className="lg:col-span-12 xl:col-span-5 space-y-8 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 items-stretch">
              <a href="mailto:hello@doctorcommerce.co"
                className="p-4 rounded-2xl backdrop-blur-md flex flex-col items-center justify-center gap-2 text-center shadow-sm transition-all cursor-pointer group border"
              ...
              >
                ...
                <div className="min-w-0 w-full">
                  <span className="block font-sans text-[10px] uppercase font-semibold tracking-wider mb-0.5" style={{ color: '#94a3b8' }}>Email us</span>
                  <span className="block font-sans font-bold text-[11px] select-all truncate" style={{ color: '#0c1a2e' }}>hello@doctorcommerce.co</span>
                </div>
              </a>

              <a href="https://wa.me/15624705073?text=Hi%20Doctor%20Commerce!%20We'd%20love%20to%20discuss%20our%20project%20requirements%20directly."
              ...
              >
                ...
                <div className="min-w-0 w-full">
                  <span className="block font-sans text-[10px] uppercase font-bold tracking-wider mb-0.5" style={{ color: '#16a34a' }}>WhatsApp</span>
                  <span className="block font-sans font-bold text-[11px] select-all" style={{ color: '#0c1a2e' }}>+1 562 470 5073</span>
                </div>
              </a>

              <a href="tel:+15624705073"
                className="p-4 rounded-2xl backdrop-blur-md flex flex-col items-center justify-center gap-2 text-center shadow-sm transition-all cursor-pointer group border"
                style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderColor: '#bae6fd' }}
                onMouseEnter={e => e.currentTarget.style.backgroundColor = '#ffffff'}
                onMouseLeave={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.6)'}
              >
                ...
                <div className="min-w-0 w-full">
                  <span className="block font-sans text-[10px] uppercase font-semibold tracking-wider mb-0.5" style={{ color: '#94a3b8' }}>Call us</span>
                  <span className="block font-sans font-bold text-[11px]" style={{ color: '#0c1a2e' }}>+1 562 470 5073</span>
                </div>
              </a>
            </div>
